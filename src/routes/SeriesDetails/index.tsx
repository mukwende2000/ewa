...
import { CastItemType, Genre, SeriesDetailsType } from "../../utils/types";

function index() {
  const data = useLoaderData() as [SeriesDetailsType, CastItemType[]];
    ...
    <main className="text-white">
      <Container styles="md:flex gap-10 px-10">
        <div className="basis-full grid md:block place-items-center">
          <img src={`${imageUrl}${series?.poster_path}`} alt={series?.name} />
        </div>
        <div className="basis-full">
          <h1 className="text-5xl text-center md:text-left">{series?.name}</h1>
          ...
          <div>
            ...
            <div className="flex gap-3">
              {series?.genre_ids?.map((id: number) => {
                return (
                  <p
                    key={id}
...
export async function loader({ params }: any): Promise<any> {
  const { id } = params;
  const data: [SeriesDetailsType, CastItemType][] = [];
  await axios
    .all([
      axios.get(
